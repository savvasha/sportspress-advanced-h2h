...
class SAH2H_Tiebreak_Criteria {
	...
	public static function regular_order_output( $post ) {
		...
		if ( is_array( $sah2h_column_order ) ) {
			...
			usort( $columns, function ($a, $b) use ( $sorting_sah2h_column_order ) {
				...
				<li>
					<div class="sah2h-item-bar sah2h-layout-item-bar">
						<div class="sah2h-item-handle sah2h-layout-item-handle ui-sortable-handle">
							...
							<input type="hidden" name="sah2h_column_order[<?php echo $i; ?>][column]" value="<?php echo esc_html( $column->post_name ); ?>">
						</div>
						
						<select class="h2h_order" name="sah2h_column_order[<?php echo $i; ?>][order]" <?php echo $disabled; ?>>
							<?php
							$options = array(
						...
						<input class="sah2h-toggle-switch" type="checkbox" name="sah2h_column_order[<?php echo $i; ?>][h2h_visibility]" id="sah2h_column_show_<?php echo esc_attr( $column->post_name ); ?>" value="1" <?php checked( $h2h_visibility, 1 ); ?> />
			...
			?>
				<li>
					<div class="sah2h-item-bar sah2h-layout-item-bar">
						<div class="sah2h-item-handle sah2h-layout-item-handle ui-sortable-handle">
							<span class="sah2h-item-title item-title"><?php echo esc_html( $column->post_title ); ?></span>
							<input type="hidden" name="sah2h_tiebreak_order[<?php echo $i; ?>][column]" value="<?php echo esc_html( $column->post_name ); ?>">
						</div>
						
						<select class="h2h_order" name="sah2h_tiebreak_order[<?php echo $i; ?>][order]" <?php echo $disabled; ?>>
							<?php
							$options = array(
								'DESC' => esc_attr__( 'Descending', 'sportspress' ),
								'ASC'  => esc_attr__( 'Ascending', 'sportspress' ),
						...
						<input class="sah2h-h2honly-checkbox" type="checkbox" name="sah2h_tiebreak_order[<?php echo $i; ?>][h2h_only]" id="sah2h_h2honly_<?php echo esc_attr( $column->post_name ); ?>" value="1" <?php checked( $h2h_only, 1 ); ?> <?php echo $disabled; ?>/>
						<label for="sah2h_h2honly_<?php echo esc_attr( $column->post_name ); ?>">H2H Only</label>
						<input type="hidden" name="sportspress_<?php echo esc_attr( $column->post_name ); ?>_visibility[]" value="0">
						<input class="sah2h-toggle-switch" type="checkbox" name="sah2h_tiebreak_order[<?php echo $i; ?>][h2h_visibility]" id="sah2h_tiebreak_show_<?php echo esc_attr( $column->post_name ); ?>" value="1" <?php checked( $h2h_visibility, 1 ); ?> />
